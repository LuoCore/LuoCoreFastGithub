...
namespace LuoCoreFastGithub
{
    ...
    public static class KestrelServerExtensions
    {
        ...
        {
            var options = kestrel.ApplicationServices.GetRequiredService<IOptions<LuoCoreFastGithubOptions>>().Value;
            var httpProxyPort = options.HttpProxyPort;

            if (GlobalListener.CanListenTcp(httpProxyPort) == false)
            {
                throw new LuoCoreFastGithubException($"tcp端口{httpProxyPort}已经被其它进程占用，请在配置文件更换{nameof(LuoCoreFastGithubOptions.HttpProxyPort)}为其它端口");
            }
        ...
        /// </summary>
        /// <param name="kestrel"></param>
        /// <exception cref="LuoCoreFastGithubException"></exception>
        ...
        private static ILogger GetLogger(this KestrelServerOptions kestrel)
        {
            ...
            return loggerFactory.CreateLogger($"{nameof(LuoCoreFastGithub)}.{nameof(HttpServer)}");
        }
