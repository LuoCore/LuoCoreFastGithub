...
namespace LuoCoreFastGithub.PacketIntercept.Dns
{
    /// <summary>
    /// 代理冲突解决者
    /// </summary>
    ...
    sealed class ProxyConflictSolver : IDnsConflictSolver
    {
        ...
        private readonly IOptions<LuoCoreFastGithubOptions> options;
        ...
        public ProxyConflictSolver(
            IOptions<LuoCoreFastGithubOptions> options,
        ...
        private void CheckProxyConflict()
        {
            ...
            foreach (var domain in this.options.Value.DomainConfigs.Keys)
            {
                ...
                if (proxyServer != null)
                {
                    this.logger.LogError($"由于系统设置了代理{proxyServer}，{nameof(LuoCoreFastGithub)}无法加速{domain}");
                }
            }
        }
