...
namespace LuoCoreFastGithub.PacketIntercept.Dns
{
    ...
    sealed class DnsInterceptor : IDnsInterceptor
    {
        ...
        public DnsInterceptor(
            LuoCoreFastGithubConfig LuoCoreFastGithubConfig,
            ILogger<DnsInterceptor> logger,
            IOptionsMonitor<LuoCoreFastGithubOptions> options)
        {
        ...
        unsafe private void ModifyDnsPacket(WinDivertPacket packet, WinDivertAddress addr)
        {
            ...
            if (this.LuoCoreFastGithubConfig.IsMatch(question.Name.ToString()) == false)
            {
                return;
            }
