...
namespace LuoCoreFastGithub
{
    ...
    static class ServiceExtensions
    {
        ...
        {
            var logger = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger(nameof(LuoCoreFastGithub));
            if (UseCommand(logger) == false)
            {
                using var mutex = new Mutex(true, "Global\\LuoCoreFastGithub", out var firstInstance);
