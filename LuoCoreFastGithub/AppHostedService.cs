...
namespace LuoCoreLuoCoreFastGithub
{
    /// <summary>
    /// app后台服务
    /// </summary>
    sealed class AppHostedService : BackgroundService
    {
        ...
        private readonly IOptions<LuoCoreFastGithubOptions> LuoCoreFastGithubOptions;
        private readonly ILogger<AppHostedService> logger;

        public AppHostedService(
            IHost host,
            IOptions<AppOptions> appOptions,
            IOptions<LuoCoreFastGithubOptions> LuoCoreFastGithubOptions,
            ILogger<AppHostedService> logger)
        {
            this.host = host;
            this.appOptions = appOptions;
            this.LuoCoreFastGithubOptions = LuoCoreFastGithubOptions;
            this.logger = logger;
        }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            await this.CheckLuoCoreFastGithubProxyAsync(stoppingToken);
            await this.WaitForParentProcessExitAsync(stoppingToken);
        }


        /// <summary>
        /// 检测LuoCoreFastGithub代理设置
        /// </summary>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        private async Task CheckLuoCoreFastGithubProxyAsync(CancellationToken cancellationToken)
        {
            if (OperatingSystem.IsWindows() == false)
            {
                try
                {
                    if (await this.UseLuoCoreFastGithubProxyAsync() == false)
                    {
                        var httpProxyPort = this.LuoCoreFastGithubOptions.Value.HttpProxyPort;
        ...
        /// <summary>
        /// 应用LuoCoreFastGithub代理
        /// </summary>
        /// <param name="proxyServer"></param>
        /// <param name="httpProxyPort"></param>
        /// <returns></returns>
        private async Task<bool> UseLuoCoreFastGithubProxyAsync()
        {
            ...
            var domain = this.LuoCoreFastGithubOptions.Value.DomainConfigs.Keys.FirstOrDefault();
            if (domain == null)
            {
                return true;
            }
